...
**Что работает:** React-интерфейс подключён к реальному API: загрузка Parquet, top-20, поиск всех gid, направленный подграф, карточки, рекомендации, временные наблюдения, AI и три серверных CSV. CLI и Swagger сохранены. Результаты — гипотезы для проверки, не доказательство виновности.
